...
function reset(){
	...
	if(numberOfCircles==1){
		slicePizza(1981,"gauche");
		slicePizza(1981,"droite");
		slicePizza(1988,"gauche");
		slicePizza(1988,"droite");
		slicePizza(1995,"gauche");
		slicePizza(1995,"droite");
		slicePizza(2007,"gauche");
		slicePizza(2007,"droite");
	}
...
function slicePizza(année,parti){
	switch(année){
		case 1981: if(parti=="gauche"){creerSlice(225);} else {creerSlice(270);} break;
		case 1988: if(parti=="gauche"){creerSlice(180);} else {creerSlice(315);} break;
		case 1995: if(parti=="gauche"){creerSlice(135);} else {creerSlice(0);} break;
		case 2007: if(parti=="gauche"){creerSlice(90);} else {creerSlice(45);} break;
	}
}
		var tabThemes, tabPoids;
function creerSlice(angle){

tabThemes=["économie","sécurité","inégalités","justice"];

  switch(angle){
          case 0:tabPoids=["9","4","17","9"];break;
          case 45:tabPoids=["15","7","17","14"];break;
          case 90:tabPoids=["7","10","23","12"];break;
          case 135:tabPoids=["10","3","11","2"];break;
          case 180:tabPoids=["4","12","10","6"];break;
          case 225:tabPoids=["2","4","6","10"];break;
          case 270:tabPoids=["9","12","10","6"];break;
          case 315:tabPoids=["3","2","3","22"];break;
  }
	...
	d3.select("svg")
	  .append("svg")
		...
		.attr("id",function(d){return angle;})
			...
			.data(tabPoids)
			.enter().append("circle")
			  .attr("id",function(d){return tabThemes[tabPoids.indexOf(d)];})
			  .attr("name","1981")
			  .attr("r",function(d){return 3*scale(d);})
			  .attr("cx",function(d){return (1+Math.sqrt(2))*3*1.5*d+(500/(Math.sqrt(2))-(2+Math.sqrt(2))*3*1.5*d)*Math.random();})
			  .attr("cy",function(d){return calculCY(tabThemes[tabPoids.indexOf(d)]);})
